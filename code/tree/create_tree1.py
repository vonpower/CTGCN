...
def create_txt(outputpath,graph_id,direct = False):
    if int(direct) == 0:
        ...
        data = trans.iloc[idx, :]
        data = data.rename(columns={'value':'to_value','fee':'to_fee'})# 修改列名
        ## 根据balance生成另外一方的value,fee
        from_value = list(data['from_balance'])
        from_fee = list(data['from_balance'])
        data['from_value'] = from_value
        data['from_fee'] = from_fee
        # filename_edge = direction + 'edge_index_' + str(graph_id) + '.txt'
        # filename_node = direction + 'tree_' + str(graph_id) + '.txt'
        ...
        data = trans.iloc[idx, :]
        data = data.rename(columns={'value': 'from_value', 'fee': 'from_fee'})  # 修改列名
        ## 根据balance生成另外一方的value,fee
        to_value = list(data['to_balance'])
        to_fee = list(data['to_balance'])
...
outputpath = 'D:/区块链/实验模型/新增交易时间和交易次数/Bi-GCN/data/tree1/'
